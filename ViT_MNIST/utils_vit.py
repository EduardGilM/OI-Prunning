...
def get_mnist_dataloaders(batch_size=32, root=".", use_distributed=None):
    ...
    if use_distributed:
        return setup_distributed_dataloaders(
...
def evaluate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        pbar = (
            tqdm(val_loader, leave=False, desc="Evaluating")
            if is_main_process()
            else val_loader
        )
        for img, label in pbar:
...
def train_model(
    model, train_loader, val_loader, epochs=1, device=None, train_sampler=None
):
    if device is None:
        device = get_device()
    ...
    optimizer = optim.Adam(
        model.parameters(), betas=ADAM_BETAS, lr=1e-4, weight_decay=ADAM_WEIGHT_DECAY
    )
...
def get_class_filtered_dataloader(dataset, target_class, batch_size, shuffle=False):
    """
    Crea un DataLoader filtrado por una clase específica.

    Args:
        dataset: Dataset completo (train o test)
        target_class: Clase a filtrar (0-9)
        batch_size: Tamaño del batch
        shuffle: Si True, mezcla las muestras de esa clase

    Returns:
        DataLoader con solo muestras de target_class
    """
...
def get_mnist_dataloaders_train_test(batch_size=32, root=".", use_distributed=None):
    """
    Solo divide en train y test (sin val split).
    ...
    if use_distributed:
        return setup_distributed_dataloaders(
            train_dataset, None, test_dataset, batch_size
        )
